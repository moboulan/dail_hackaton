// Licence status, the modules, attestations. Nothing to read, one action per module.
...
function cardStatus(progress) {
  ...
  const step = STEPS.find((s) => s.id === progress.lastStep) ?? STEPS[0];
...
export default {
  ...
  render({ state, notice }) {
    ...
    const attestations = validated.length
      ? validated.map((m) => esc(m.title)).join(", ")
      : "aucune";
    ...
    return `
      ...
      <p class="attestations">Attestations : ${attestations}</p>`;
  },
