...
export const PERSONAS = {
  ...
  rhume: {
    ...
    facts: {
      tension: "quand tu dis que tu prends un traitement pour la tension",
      sansdiabete: "quand tu dis que tu n'es pas diabétique",
    },
  ...
  soleil: {
    ...
    facts: { brulure: "quand tu décris ta brûlure ou ton coup de soleil" },
  ...
  ventre: {
    name: "M. Driss",
    languageMatters: true,
    facts: { alerte: "quand tu dis que la douleur est forte en bas à droite ou que tu as de la fièvre" },
...
function factRules(persona) {
  const lines = Object.entries(persona.facts).map(([id, when]) => `- ${when} : ajoute [FAIT:${id}] à la fin de ton message, une seule fois dans la conversation.`);
