...
{"criteria":[{"id":"langue","score":0,"comment":"...","quote":"...","better":"..."}, ...les 5 critères dans l'ordre...],"summary":"...","quiz":[{"question":"...","options":["...","...","..."],"correct":0,"reasons":["...","...","..."]}, ...3 questions...]}
Règles :
...
- "better" : si la note est inférieure à 2, une phrase que le pharmacien aurait pu dire (moins de 25 mots) ; sinon "". Elle applique toujours la bonne décision du cas (jamais un produit contre-indiqué) et n'ajoute aucun fait absent des fiches.
- "summary" : une phrase adressée au pharmacien en le vouvoyant ("Vous..."), sur le point le plus important à retenir.
- score null : seulement si la situation ne demandait pas ce critère. Pour "langue" : obligatoire (jamais null) si ce que le pharmacien savait avant ou ce que dit le client indique une préférence de langue ; null seulement si rien n'indiquait de préférence ni de difficulté. Pour "complement" : null si aucun complément n'avait de sens. "questions" et "produit" ne sont jamais null. Avec null, "comment", "quote" et "better" sont "".
- "quiz" : 3 questions à choix unique pour s'entraîner sur ce que le pharmacien a manqué ou mal fait (sinon sur les points clés du cas). Chaque question : une situation courte au comptoir, 3 réponses dont une seule juste, "correct" = index de la bonne, "reasons" = pour chaque réponse une phrase courte qui explique pourquoi elle est juste ou non. N'utilise que les faits des fiches produits ; aucun dosage, aucun médicament réel. Vouvoie le pharmacien.`;
}
...
export async function handleDebrief(body, apiKey, model) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const grading = validateGrading(raw, body.messages, PERSONAS[body.module]);
