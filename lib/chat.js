...
export async function handleChat(body, apiKey, model) {
  ...
  try {
    raw = await complete({
      apiKey,
      model,
      messages: [{ role: "system", content: systemPrompt(body.module) }, ...toModelMessages(body.messages)],
      maxTokens: 200,
      temperature: 0.8,
    });
  } catch (error) {
  ...
  const known = PERSONAS[body.module].facts;
  const facts = [...raw.matchAll(/\[FAIT:([a-z]+)\]/g)].map((m) => m[1]).filter((id) => id in known);
  const reply = raw.replaceAll(END_MARK, "").replace(/\[FAIT:[a-z]*\]/g, "").trim();
  return { status: 200, body: { reply, left, facts: [...new Set(facts)] } };
}
