...
| P02 | Feedback is one sentence per turn, identical for every choice, so it praises weak replies ("Ce document est pourtant très clair" is followed by praise). | 03 | High | fixed | The Bilan grades each conversation on 5 criteria, with a comment per criterion that matches its score (step 5). |
...
| P04 | A weak reply gets no explanation, no better alternative and no retry. | browser | Medium | fixed | Each missed criterion shows what was missing and a better phrasing (step 5). |
| P05 | The best reply is always option 1, in every dialogue turn and every quiz, so the right answer can be guessed. | code | Medium | fixed | Right answers sit at varying positions in all 3 quizzes (steps 3 and 6). |
...
| ID | Problem | Evidence | Sev | Status | Fix (commit) |
| --- | --- | --- | --- | --- | --- |
| P08 | The result shows a score (0 %) but not which replies were weak or why. | 04 | High | fixed | The Bilan shows every criterion with its level, the pharmacist's own words and what to say instead (step 5). |
...
| P10 | The quiz sits inside the result page instead of being a clear final step. | 04 | Medium | fixed | The quiz is its own last step, after the Bilan (step 6). |
| P11 | The quiz marks right and wrong but never explains why. | 04 | Medium | fixed | Every answer shows a one-line reason (step 6). |
| P12 | Quiz answers lock after one click. The only way back is a full reset. | browser | Medium | fixed | Answers can be changed until right; only the first answer counts for the score (step 6). |
| P13 | Answered quiz options turn pale grey (disabled styling), hard to read. | 04 | Low | fixed | Answered options keep full contrast; right and wrong are marked by colour and border (step 6). |

## Progress and navigation

| ID | Problem | Evidence | Sev | Status | Fix (commit) |
| --- | --- | --- | --- | --- | --- |
| P14 | Progress counts clicks, not learning: 78 % with a 0 % dialogue score and a wrong quiz answer. | 05 | High | fixed | The module score is 50 % Bilan + 50 % Quiz, first answers only; the dashboard counts validated modules, not clicks (step 6). |
...
| ID | Problem | Evidence | Sev | Status | Fix (commit) |
| --- | --- | --- | --- | --- | --- |
| P23 | Ticking a preparation box re-renders the page and sends keyboard focus to `<body>`. | browser (`document.activeElement` = BODY) | Medium | fixed | Focus returns to the chosen answer after each quiz answer (steps 2 and 6). |
