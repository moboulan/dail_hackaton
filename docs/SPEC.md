...
| 19 Sep | Accounts and roles for the demo: a login page (two pharmacists, one manager, passwords in the page, so a demonstration of roles and not security). Pharmacists see only their own register and modules; the manager sees only Équipe: each pharmacist's level per module, licence count and printable attestations. Data stays on the pharmacy computer. |
| 19 Sep | "Créer un cas" removed from the product before the demo (the manager page shows the team and attestations only); the server still accepts custom cases but nothing in the app sends them. |
| 19 Sep | "Créer un cas": the pharmacy writes new cases (client, hidden facts, 3 products, correct decision). Saved in the browser, listed in the register, played and graded by the same AI flow; the server validates every field (`lib/cases.js`). Custom cases use a generic fallback quiz and no live product verdicts; they do not count toward the licence. Staff overview not built: it needs accounts and a database. |
| 19 Sep | Revealed facts can also confirm a product (no diabetes: Gorgéa Miel possible). Facts come from the model's tag or the reply's own words. |
| 19 Sep | Grader judges the whole conversation and gives 2 when the essentials are done; quiz questions have exactly one clearly right answer. |
| 19 Sep | Quiz corrections shown on the quiz page after submission (icons only); Résultat keeps the score, the bars and the actions. |
| 19 Sep | Adaptive Mémo: a réflexe missed in any Bilan shows a short example sentence instead of the generic line, until it is mastered. |
| 19 Sep | Bilan shows the real exchange only for criteria not fully met. |
...
2. **Échange:** the AI customer speaks first (Mme Imane in darija). Products are ruled out or
   confirmed live on the shelf as facts come out; the Mémo adapts to past misses. Mémo (the 4 réflexes) on the left, the
